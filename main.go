...
NOTE: The --certfile and --keyfile must both be provided to serve HTTPS. When serving HTTPS

SYNOPSIS:

    codenames [--quiet | [--force | --verbose]]
              --adjectives FILE --animals FILE
              [--http NUMBER] [--https NUMBER]
              [--certfile FILE] [--keyfile FILE] [--redirect]
...
EXAMPLES:
    ...
    codenames --adjectives adjectives.txt --animals animals.txt --
    codenames --adjectives adjectives.txt --animals animals.txt --certfile $HOME/.local/share/mkcert/rootCA.pem --keyfile $HOME/.local/share/mkcert/rootCA-key.pem --redirect
